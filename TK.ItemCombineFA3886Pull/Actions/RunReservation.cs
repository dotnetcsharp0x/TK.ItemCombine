    ...
    public class RunReservation : IDisposable
    {
        ...
        public async Task<double?> CombineManual(Params? prm)
        {
            ...
            using (IDbConnection db = new OracleConnection(connectionString))
            {
                ...
                Console.WriteLine("FA3886_ITEM.Count(): " + FA3886_ITEM.Count());
                    foreach (var item in FA3886_ITEM)
                    {

                        item.IMPORT = 1;
                        Console.WriteLine("Order: " + item.FA3886_BS_NR + " pos_nr: " + item.FA3886_POSTEN_ID);
                    ...
                    try { 
                        ...
                        string sql = "INSERT INTO TKMIRATORG.FA3886_00101 (" +
                            "FA3886_PROG_NR" +
                            ", FA3886_AUSW_NR" +
                            ", FA3886_PRIO" +
                            ", FA3886_ART_KOMM_ID" +
                            ...
                            ", :FA3886_BEARB_LFD_NR" +
                            ", :FA3886_FREIE_NR_1)";
                        var rowsAffected = db.Execute(sql, item);
                        Console.WriteLine($"{rowsAffected} row(s) inserted.");
                        rowsAffected = db.Execute("update TKMIRATORG.FA3886_00101_ITEM set IMPORT = :val where FA3886_AUTO_INC = :FA3886_AUTO_INC", new { val = 1, FA3886_AUTO_INC = item.FA3886_AUTO_INC });
                        Console.WriteLine($"{rowsAffected} row(s) updated.");
                    }
                    catch (Exception ex)
                    {
                        //db.BulkUpdate(FA3886_ITEM);
                        var rowsAffected = db.Execute("update TKMIRATORG.FA3886_00101_ITEM set IMPORT = :val where FA3886_AUTO_INC = :FA3886_AUTO_INC", new { val = 1, FA3886_AUTO_INC = item.FA3886_AUTO_INC });
                        Console.WriteLine($"{rowsAffected} row(s) updated.");
                        Console.WriteLine("BS_NR: " + item.FA3886_BS_NR + " pos_nr: " + item.FA3886_POSTEN_ID + " error");
                    }
                    finally { }
                }
                    foreach (var item in FA3887_ITEM)
                    {
                       ...
                       ,
                                      FA3887_ZUORD_TYP = i.FA3887_ZUORD_TYP
                       ,
                    ...
                    try {
                        ...
                        var sql = "INSERT INTO TKMIRATORG.FA3887_00102 (" +
                            "FA3887_PROG_NR" +
                            ", FA3887_AUSW_NR" +
                            ", FA3887_PRIO" +
                            ", FA3887_ART_KOMM_ID" +
                        ...
                        var rowsAffected = db.Execute(sql, item);
                        Console.WriteLine($"{rowsAffected} row(s) inserted.");
                        rowsAffected = db.Execute("update TKMIRATORG.FA3887_00102_ITEM set IMPORT = :val where FA3887_ART_KOMM_ID = :FA3887_ART_KOMM_ID AND FA3887_OBER_ID = :FA3887_OBER_ID", new { val = 1, FA3887_ART_KOMM_ID = item.FA3887_ART_KOMM_ID, FA3887_OBER_ID = item.FA3887_OBER_ID });
                        Console.WriteLine($"{rowsAffected} row(s) updated.");
                    }
                    catch (Exception ex)
                    {
                        var rowsAffected = db.Execute("update TKMIRATORG.FA3887_00102_ITEM set IMPORT = :val where FA3887_ART_KOMM_ID = :FA3887_ART_KOMM_ID AND FA3887_OBER_ID = :FA3887_OBER_ID", new { val = 1, FA3887_ART_KOMM_ID = item.FA3887_ART_KOMM_ID, FA3887_OBER_ID = item.FA3887_OBER_ID });
                        Console.WriteLine($"{rowsAffected} row(s) updated.");
                        Console.WriteLine(item.FA3887_OBER_ID + " error");
                    }
                    finally { }
                }
            }
